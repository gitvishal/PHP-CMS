...
$edit_page = 'SELECT * FROM pages WHERE p_id=:p_id';
$database->query($edit_page);
$database->bind(':p_id', $_POST['page_id'],PDO::PARAM_INT); 
$edit_p=$database->single();

$parent = 'SELECT * FROM pages WHERE user_id=:author_id AND rank <>:rank AND status <> :status AND p_id <>:p_id';
$database->query($parent);
$database->bind(':author_id', SJ_USER_ID,PDO::PARAM_INT); 
$database->bind(':rank', RANK_3);
$database->bind(':status',DRAFT);
$database->bind(':p_id', $edit_p['p_id'],PDO::PARAM_INT);
...
<body>
            ...
            <div class="container-fluid">
              ...
              <form role="form" id="publish_page" method="post" action="publish.php">
                <div class="form-group">
                  ...
                  name="page" id="page" placeholder="Enter page name" value="<?php echo $edit_p['title']; ?>">
                </div>

                <div class="form-group">
                  <label for="ckeditor">Content</label>
                  <textarea class="form-control ckeditor" id="editor" name="editor" >
                  </textarea>
                </div>
                <div class="form-group">
                  <label for="datetimepicker">Publish time</label>
                
                  <input type="text" class="form-control" value="<?php echo $edit_p['publish']; ?>" id="datetimepicker" name="datetimepicker"/>
                </div>
                <div class="form-group">
                    <label class="screen-reader-text" for="parent_id">Parent</label>
                       ...
                       <input type="hidden" name="page_id" id="page_id" value="<?php echo $edit_p['p_id']; ?>">
                       <input type="hidden" name="page_content" id="page_content" value="<?php echo $edit_p['data']; ?>">                 
              ...
              <div class="col-sm-3">
                <div class="panel panel-default">
                    <div class="panel-body">
                        <button type="button" class="btn btn-default btn-md">Preview</button>
                    </div>
                </div>
    ...
    $(document).ready(function(){
        CKEDITOR.instances.editor.setData($("#page_content").val());
        function draftData()
        {
        ...
        $('#parent_id ,#datetimepicker,form input[type="text"]').one("change",function(){
            draftData();
            draftInterval();
        });
        ...
        /*$('#'+ CKEDITOR.instances.editor.id).one('change', function() { 
            draftData();
            draftInterval(); 
        });
*/
